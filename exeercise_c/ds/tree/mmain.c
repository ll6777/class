...
static void _draw(const struct node_st *root, int level) //画出树
{
	if (root == NULL)
		return ;
...
static struct node_st **_deleteFind(struct node_st **root, int id)  //查找
{
	if (*root == NULL)
		return NULL;
	if ((*root)->data.id == id)
...
static struct node_st *treeMaxNode(const struct node_st *root) //找最大的
{
	if (root == NULL)
		return NULL;
	if (root->right == NULL)
...
static int treeDelete(struct node_st **root, int id) //删除
{
